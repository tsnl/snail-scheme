...
//    define_builtin_variadic_arithmetic_fn<int_sub_cb, float_sub_cb>("-");
    ...
    C_word rib_pair = cons(m_init_var_rib, m_init_elt_rib);
...
void VirtualMachine::define_builtin_fn(
    ...
    C_word var_obj = new SymbolObject(intern(std::move(name_str)));
    C_word vars_list = nullptr;
    for (size_t i = 0; i < arg_names.size(); i++) {
        vars_list = cons(new SymbolObject(intern(arg_names[i])), vars_list);
    }
    C_word elt_obj = new EXT_CallableObject(callback, m_init_env, vars_list);
    ...
    define_builtin_fn(
        ...
        [=](C_word args) -> C_word {
            ...
            if (arg_count == 1) {
                ...
                // adding two integers:
                auto aa = extract_args<2>(args);
                auto res = static_cast<C_word>(aa[0])->value();
                int_fold_cb(res, static_cast<C_word>(aa[1])->value());
                ...
                auto aa = extract_args<2>(args);
                auto res = static_cast<C_word>(aa[0])->value();
                float_fold_cb(res, static_cast<C_word>(aa[1])->value());
                ...
                C_word first_arg = car(rem_args);
                rem_args = cdr(rem_args);

                C_float unwrapped_accum;
                if (first_arg->kind() == ObjectKind::Integer) {
                    unwrapped_accum = static_cast<C_float>(static_cast<C_word>(first_arg)->value());
                } else {
                    unwrapped_accum = static_cast<C_word>(first_arg)->value();
                }
                for (; rem_args; rem_args = cdr(rem_args)) {
                    C_word operand = car(rem_args);
                    C_float v;
                    if (operand->kind() == ObjectKind::Integer) {
                        v = static_cast<C_float>(static_cast<C_word>(operand)->value());
                    } else {
                        // must be a float-- checked before.
                        v = static_cast<C_word>(operand)->value();
                    }
                ...
                C_float unwrapped_accum = static_cast<C_word>(first_arg)->value();
                for (; rem_args; rem_args = cdr(rem_args)) {
                    C_word operand = car(rem_args);
                    // must be a float-- checked before.
                    C_float v = static_cast<C_word>(operand)->value();
                    float_fold_cb(unwrapped_accum, v);
                }
                ...
                C_word unwrapped_accum = static_cast<C_word>(first_arg)->value();
                for (; rem_args; rem_args = cdr(rem_args)) {
                    C_word operand = car(rem_args);
                    // 'v' must be an integer-- checked before.
                    C_word v = static_cast<C_word>(operand)->value();
...
C_word VirtualMachine::lookup(C_word symbol, C_word env_raw) {
    ...
    if (symbol->kind() != ObjectKind::Symbol) {
        ...
        IntStr sym_name = static_cast<C_word>(symbol)->name();
        ...
        for (
            C_word rem_ribs = env;
            rem_ribs;
            rem_ribs = static_cast<C_word>(rem_ribs->cdr())
        ) {
            auto rib_pair = static_cast<C_word>(rem_ribs->car());
            auto variable_rib = static_cast<C_word>(rib_pair->car());
            auto value_rib = static_cast<C_word>(rib_pair->cdr());
            ...
            for (
                ...
                rem_value_rib;
                ((rem_variable_rib = static_cast<C_word>(rem_variable_rib->cdr())),
                 (rem_value_rib = static_cast<C_word>(rem_value_rib->cdr())))
            ) {
                assert(rem_variable_rib && "Expected rem_variable_rib to be non-null with rem_value_rib");
                auto variable_rib_head = static_cast<C_word>(rem_variable_rib->car());
...
C_word VirtualMachine::continuation(C_word s) {
    static C_word nuate_var = new SymbolObject(intern("v"));
